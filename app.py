...
# ─────────────────────────────────────────────
# HTML embed helper (auto-resizes to content)
# ─────────────────────────────────────────────
def components_html_autoresize(html: str):
    """Render HTML in Streamlit and auto-resize the iframe to fit content."""
    wrapped = f"""
{html}
<script>
(function() {{
  ...
  // Recompute after small delays (fonts/layout)
  setTimeout(sendHeight, 250);
  setTimeout(sendHeight, 750);
  setTimeout(sendHeight, 1500);
  // Observe DOM changes (tabs/accordion inside your HTML)
  try {{
    var obs = new MutationObserver(function() {{ sendHeight(); }});
    obs.observe(document.body, {{ childList: true, subtree: true, attributes: true }});
  }} catch(e) {{ }}
}})();
</script>
"""
    components.html(wrapped, height=120, scrolling=False)
...
with st.sidebar:
    ...
    if api_key:
        ...
        st.info("Set it in Streamlit Cloud → Manage app → Settings → Secrets.")
        st.stop()
...
""", unsafe_allow_html=True)
    ...
    with tab4:
        ...
        if html_path.exists():
            ...
            components_html_autoresize(html_content)
        else:
            st.warning(
