...
with st.sidebar:
    ...
    # Developer info
    st.markdown("""
<div style="background:rgba(255,255,255,0.1);border-radius:8px;padding:12px 14px;margin-bottom:8px;">
<div style="font-size:13px;font-weight:700;color:rgba(255,255,255,0.95);margin-bottom:4px;">Jagadish Thiruvayipati</div>
<div style="font-size:11px;color:rgba(255,255,255,0.75);margin-bottom:2px;">Student ID: XW014  |  EMBA 25-27</div>
<div style="font-size:11px;color:rgba(255,255,255,0.75);">IIM Ranchi — Work-Applied Intelligence</div>
</div>
""", unsafe_allow_html=True)

    st.markdown("---")

    # API Key — loaded from Streamlit Secrets, no user input needed
    try:
    ...
    st.markdown(f"**Queries:** {st.session_state.query_count} / {QUERY_LIMIT}")
    st.progress(min(st.session_state.query_count / QUERY_LIMIT, 1.0))
    remaining = QUERY_LIMIT - st.session_state.query_count
    if remaining > 0:
        st.markdown(f"<small style='color:rgba(255,255,255,0.6);'>{remaining} queries remaining this session</small>", unsafe_allow_html=True)
    else:
        st.error("Session limit reached. Refresh to continue.")
...
# ─────────────────────────────────────────────
# Project Insights — collapsible HTML visualisation
# ─────────────────────────────────────────────
with st.expander("📊 Project Insights — Before vs After AI, RAG Architecture & Impact Analysis", expanded=False):
    st.markdown("""
<div style="font-size:13px;color:#6b7280;margin-bottom:12px;">
This interactive visualisation shows the full project context: the manual workflow this system replaces,
how the RAG pipeline works, and the measurable impact on resolution time and consistency.
</div>
""", unsafe_allow_html=True)
    HTML_CONTENT = open(Path(__file__).parent / "ai_workflow_visuals.html").read()
...
if search_btn and query_input.strip():
    if not api_key:
        ...
        st.warning(f"⏳ Session limit of {QUERY_LIMIT} queries reached. Please refresh the page to start a new session.")
    else:
