...
# ─────────────────────────────────────────────
# API Key — resolved at top level before anything else
# ─────────────────────────────────────────────
try:
...
.excerpt-box {
    ...
    color: #4a4640; line-height: 1.6; font-style: italic;
}
.answer-box {
...
def tokenize(text: str) -> list:
    ...
    stopwords = {
        ...
        'just','of','at','by','for','with','about','against','between','into','through',
        'during','before','after','above','below','to','from','up','down','in','out',
        'on','off','over','under','again','further','then','once','and','but','or','if','as'
    }
...
def get_verbatim_score(query: str, rule_text: str) -> float:
    ...
    return 0.0 if not q else len(q & set(tokenize(rule_text))) / len(q)

def confidence_label(score: float) -> str:
    return "HIGH" if score >= 0.35 else "MEDIUM" if score >= 0.18 else "LOW"

def retrieve(query: str, docs: list, doc_vectors: list, df: dict, N: int, top_k: int = 5) -> list:
    qvec = compute_query_vector(query, df, N)
    ...
    for doc, vec in zip(docs, doc_vectors):
        ...
        final       = min(sim + tag_boost + title_boost, 1.0)
        scored.append({
            **doc,
...
def generate_answer(query: str, retrieved: list, client: Anthropic) -> str:
    context = build_context(retrieved)
    system_prompt = (
        "You are InsureIQ, an expert insurance claims specialist. "
        "Answer using ONLY the provided policy documents. "
        "Cite sources using [SOURCE N] notation. "
        "Show financial calculations step-by-step. "
        "Flag exclusions and conditions that apply. "
        "Be factual, precise, and helpful."
    )
    user_message = f"Member Question: {query}\n\nRelevant Policy Documents:\n{context}\n\nAnswer based strictly on the above documents."
...
# ─────────────────────────────────────────────
defaults = {
    "query": "", "results": None, "answer": None,
    "pipeline_log": [], "top_k": 5,
    "last_query_time_ms": None, "query_count": 0,
}
...
with st.sidebar:
    ...
    st.markdown("""
<div style="background:rgba(255,255,255,0.1);border-radius:8px;padding:12px 14px;margin-bottom:4px;">
  <div style="font-size:14px;font-weight:700;margin-bottom:4px;">Jagadish Thiruvayipati</div>
  <div style="font-size:11px;opacity:0.8;">Student ID: XW014  |  EMBA 25-27</div>
  <div style="font-size:11px;opacity:0.8;margin-top:2px;">🎓 IIM Ranchi</div>
  <div style="font-size:10px;opacity:0.55;margin-top:4px;">Work-Applied Intelligence (WAI)</div>
</div>
""", unsafe_allow_html=True)
    ...
    if api_key:
        st.markdown("✅ **Connected** — Ready to use")
    else:
        st.error("Add ANTHROPIC_API_KEY to Streamlit Secrets.")

    st.markdown("---")
    st.markdown("### 📊 Session Usage")
    st.markdown(f"**{st.session_state.query_count} / {QUERY_LIMIT}** queries used")
    st.progress(min(st.session_state.query_count / QUERY_LIMIT, 1.0))
    remaining = QUERY_LIMIT - st.session_state.query_count
    if remaining > 0:
        st.caption(f"{remaining} queries remaining this session")
    else:
    ...
    st.markdown("---")
    st.markdown(f"**Total rules:** {len(docs)}")
    st.markdown(f"**Vocabulary:** {len(df_map):,} terms")
    st.markdown("---")
    st.markdown("### 📖 About")
    st.markdown("""
**Full RAG pipeline:**
1. TF-IDF cosine similarity retrieval
2. Claude Sonnet grounded reader

No vector database — pure Python.
    """)
...
with col4:
    t_disp = f"{st.session_state.last_query_time_ms}ms" if st.session_state.last_query_time_ms else "—"
    st.markdown(f'<div class="metric-tile"><div class="metric-val">{t_disp}</div><div class="metric-label">Last Query Time</div></div>', unsafe_allow_html=True)
...
with left_col:
    ...
    query_input = st.text_area(
        label="question", value=st.session_state.query, height=110,
        placeholder="E.g. I'm 45 with diabetes needing $75,000 cardiac surgery out-of-network. What will my insurance pay?",
    ...
    if clear_btn:
        for k in ["query", "results", "answer", "pipeline_log"]:
            st.session_state[k] = "" if k == "query" else None if k in ["results", "answer"] else []
        st.rerun()

with right_col:
    st.markdown("### ⚡ Sample Questions")
    for sample in SAMPLE_QUERIES:
        if st.button(sample["label"], key=f"sq_{sample['label']}", use_container_width=True):
            st.session_state.query = sample["query"]
            st.rerun()

# ─────────────────────────────────────────────
# Project Insights — always visible
# ─────────────────────────────────────────────
st.markdown("---")
st.markdown("""
<div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;">
  <span style="font-size:22px;">📊</span>
  <span style="font-size:18px;font-weight:700;color:#1a3c5e;">Project Insights</span>
</div>
<div style="font-size:13px;color:#6b7280;margin-bottom:14px;">
  Before vs After AI &nbsp;·&nbsp; RAG Pipeline Architecture &nbsp;·&nbsp; Impact Analysis
</div>
""", unsafe_allow_html=True)
...
if html_path.exists():
    ...
    st.info("ai_workflow_visuals.html not found — add it to the app directory.")
st.markdown("---")
...
if search_btn and query_input.strip():
    if not api_key:
        ...
        st.warning(f"⏳ Session limit of {QUERY_LIMIT} queries reached. Refresh to continue.")
    else:
        ...
        pipeline_log.append({"step":1,"icon":"🔤","name":"Query Tokenization",
            "detail": f"Extracted {len(tokens)} tokens: {', '.join(tokens[:12])}{'...' if len(tokens)>12 else ''}"})

        qvec = compute_query_vector(query_input, df_map, N_docs)
        pipeline_log.append({"step":2,"icon":"🔢","name":"TF-IDF Query Embedding",
            "detail": f"Query vector: {len(qvec)} unique terms | Vocabulary coverage: {len(qvec)}/{len(df_map)}"})

        pipeline_log.append({"step":3,"icon":"📐","name":"Cosine Similarity Search",
            "detail": f"Scoring against {len(docs)} document vectors across {len(df_map):,} terms..."})
        ...
        pipeline_log.append({"step":4,"icon":"🎯","name":f"Top-{top_k} Result Retrieval",
        ...
        pipeline_log.append({"step":5,"icon":"📊","name":"Verbatim & Confidence Scoring",
            "detail": "Scores: " + " | ".join(score_parts)})

        pipeline_log.append({"step":6,"icon":"🤖","name":"Claude Reader (Answer Generation)",
        ...
        pipeline_log.append({"step":7,"icon":"✅","name":"Pipeline Complete",
...
<div style="font-size:13px;color:#374151;line-height:1.7;">{answer.replace(chr(10),'<br>')}</div>
        ...
        src_cols = st.columns(min(len(results), 5))
        for col, res in zip(src_cols, results):
            with col:
...
<span class="score-badge {conf_cls}">{conf_emoji} {r['confidence']}</span>
                ...
                if r.get("tags"):
                    tags_html = " ".join([
                        f'<span style="background:#f0f0eb;border:1px solid #e4e0d8;padding:2px 8px;border-radius:4px;font-size:10px;font-family:monospace;">{t}</span>'
                        for t in r["tags"][:8]
                    ])
...
<p style="color:#6b7280;font-size:14px;">Enter a question above or click a sample query to see the full RAG pipeline in action.</p>
</div>""", unsafe_allow_html=True)
